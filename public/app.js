...
function renderRegions() {
  ...
  $('regions').innerHTML = regions.map((item) => `
    <button class="region ${item.id === state.region ? 'selected' : ''}" data-region="${item.id}" role="radio" aria-checked="${item.id === state.region}">
      <span class="flag">${escapeHtml(item.flag || item.id.toUpperCase())}</span>
      <span class="meta"><strong>${escapeHtml(item.label)}</strong><small>${item.count} 个节点</small></span>
    </button>`).join('');
...
function updateCurrent() {
  ...
  $('activeGroup').textContent = group ? `代理组：${group.name} · ${group.nodeCount} 个可用节点` : '请选择一个代理组';
}
...
async function loadStatus() {
  $('message').textContent = '正在同步 Controller 状态...';
  ...
  try {
    ...
    $('startupButton').textContent = data.startup?.enabled ? `开机启动已开启${data.startup.source === 'scheduled-task' ? '（任务计划）' : ''}` : '开机启动';
    ...
    const readonly = (data.detectedClients || []).filter((client) => client.online && client.writable === false).map((client) => client.name).join('、');
    $('clientInfo').textContent = readonly ? `已检测到只读客户端：${readonly}` : '选择一个在线 Controller 后即可测速和切换。';
    ...
    setConnection(true, 'Controller 已连接');
    $('message').textContent = '状态已同步，可以开始测速。';
    renderRegions();
  } catch (error) {
    state.status = null;
    setConnection(false, 'Controller 未连接');
    $('activeNode').textContent = '无法连接 Mihomo Controller';
    $('activeGroup').textContent = '请确认 Clash/Mihomo 客户端正在运行，并已开放 Controller API';
...
function renderResults(data, meta = {}) {
  ...
  $('results').innerHTML = data.results.map((item, index) => {
    ...
    return `<div class="result ${index === 0 && item.ok ? 'best' : ''}">
      ...
      <div>
        <div class="node-name">${escapeHtml(item.name)}${index === 0 && item.ok ? '<span class="badge">最快</span>' : ''}</div>
        <div class="node-sub">${active ? '<span class="active-badge">当前已启用</span>' : item.ok ? '测速成功' : escapeHtml(item.error || '连接失败')}</div>
      </div>
...
async function optimize() {
  ...
  try {
    const response = await fetch('/api/optimize', {
      ...
      body:JSON.stringify({ group:state.group, region:state.region, switch:$('autoSwitch').checked, testUrl:$('testUrl').value, timeout:Number($('timeout').value) })
    });
    const data = await response.json();
    if (!response.ok) {
...
$('groupSelect').addEventListener('change', (event) => {
  ...
  $('resultCount').textContent = '等待测速';
  renderRegions();
  renderPersistedResults(state.status);
});
$('backendSelect').addEventListener('change', async (event) => {
  await fetch('/api/automation', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ action:'backend', value:event.target.value }) });
  state.group = '';
  state.region = '';
  loadStatus();
});
$('startupButton').addEventListener('click', async () => {
  ...
  try {
    ...
    const data = await response.json();
    if (!response.ok) throw new Error(data.error);
    $('message').textContent = data.enabled ? '开机启动已开启。' : '开机启动已关闭。';
    await loadStatus();
  } catch (error) {
    $('message').textContent = error.message;
    $('message').className = 'message error';
...
$('monitorOnly').addEventListener('change', async (event) => {
  await fetch('/api/automation', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ action:'monitor', value:event.target.checked }) });
  loadStatus();
});
$('lockButton').addEventListener('click', async () => {
  const locked = $('lockButton').textContent.includes('解除');
  await fetch('/api/automation', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ action:locked ? 'unlock' : 'lock' }) });
  loadStatus();
});

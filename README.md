...
- 支持 Google/OpenAI 保通检查：优先测试实际生效的出站链路，当前链路超时时才切换承载真实节点的 Selector。
...
自动优选循环还会每 1 分钟做一次轻量保通检查：只测试 Google/OpenAI 当前实际出站；如果当前出站正常，不测试其他节点。如果当前出站失败，程序会先沿着 Selector 链找到真正承载节点的组，例如 `AI网站 -> 节点选择 -> 当前节点` 会优先切换 `节点选择`，不会一开始就改 `AI网站` 或 `漏网之鱼`。只有当前组没有可用节点时，才跨地区寻找备用节点。
...
控制台可以调整切换阈值、采样次数和手动保护时间，并提供仅监控模式、临时锁定、最近自动测速排名和延迟趋势。保通检查的默认间隔是 1 分钟，单次目标超时是 5000 毫秒；这些设置会保存在本地状态中。

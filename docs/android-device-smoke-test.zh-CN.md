...
3. 在“配对 / 优选 / 结果”之间切换，确认页面不再是一条长表单。
4. 设置后台优选间隔，例如 3 分钟。
5. 点“开始测速并优选”。
6. 期望看到：弹窗报告、测速结果列表、最快节点标记，并已切换到最快节点，或提示当前已是最快节点。
5. 打开 Clash Meta，确认对应 Selector 当前节点已经变化。
8. 点“开启后台优选”，通知栏应出现 Node Pilot 前台服务。
9. 后台节点优选应按页面设置的分钟间隔运行。
10. 点“停止后台优选”应停止前台服务，但保留配对。
11. 点“撤销配对”应清空本地配对状态并停止服务。

## 必测项

- Controller 可达：探测能读到 `/version` 和 `/proxies`。
- 弹窗报告：探测、配对、失败和节点优选完成后，即使用户滑到页面下方，也能看到弹窗报告。
- Secret 错误：应提示需要 Secret 或节点优选失败，不输出 Secret。
